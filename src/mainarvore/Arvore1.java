...
class Arvore1 {
    ...
    public void inserir(Item v) {
        No novo = new No(v); // cria um novo Nó
        novo.dados = v; // atribui o valor recebido ao item de dados do Nó
        novo.dir = null;
        novo.esq = null;
        ...
        if (root == null) {
            ...
            while (true) {
                anterior = atual;
                if (v.getNome().compareTo(atual.dados.getNome()) < 0) { // ir para esquerda
                    atual = atual.esq;
                    if (atual == null) {
                        anterior.esq = novo;
                        return;
    ...
    public No buscar(Item chave) {
        if (root == null) {
            return null; // se arvore vazia
        }
        No atual = root;  // começa a procurar desde raiz
        while (atual.dados != chave) { // enquanto nao encontrou
            if (chave.getNome().compareTo(atual.dados.getNome()) < 0) {
    ...
    public boolean remover(Item v) {
        ...
        // ****** Buscando o valor **********
        while (atual.dados != v) { // enquanto nao encontrou
            pai = atual;
            if (v.getNome().compareTo(atual.dados.getNome()) < 0) { // caminha para esquerda
                atual = atual.esq;
    ...
    public void caminhar() {
        ...
        if (root != null) {  // se arvore nao esta vazia
            System.out.print("\n Valor minimo: " + min().dados);
            System.out.println("\n Valor maximo: " + max().dados);
        }
    }
